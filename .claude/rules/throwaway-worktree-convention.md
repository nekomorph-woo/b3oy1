...
- **Commit freely with `snap`.** In a destination worktree the AI commits with `snap` by default — it is the pure-change committer. `b3oy1-commit` is the **version maintainer**: only the user calls it, when a commit must also bump a plugin version.
